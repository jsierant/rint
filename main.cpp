...
//   mtu8[tu8(3)] = 56;
//   v2 = tu8(5);
  rint::ranged_int<std::numeric_limits<std::int32_t>::min(),
    std::numeric_limits<std::int32_t>::max()> a{};
  std::cerr << "JS: type:" <<boost::core::demangle(typeid(decltype(a)::value_type).name()) << std::endl;
  std::cout << "JS min:" << std::numeric_limits<std::uint32_t>::min() << std::endl;
